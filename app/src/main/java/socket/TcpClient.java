...
import com.example.along.agvcontrol0412.StereoDisplay;
...
import model.Region;
import common.StereoPoint;
import common.NavigationMessage;
import message.TcpMessage;
import message.TcpSend;
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    while (runFlag) {
//                        receive_battery_power();  //接收电量
                        byte[] dataBuff = new byte[3];
                        try {
                            in.read(dataBuff, 0, 3);
                            Log.i("tag","————————————————————————开始接收帧头,"+"0--"+(dataBuff[0]&0xff)+",1--"+(dataBuff[1]&0xff)+",2--"+(dataBuff[2]&0xff));

                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        if (MainActivity.isReceive_flag()) { //建图模式
                            ctl_showpic(dataBuff);

                            if (voltage != 0) {
                                handler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        Control_getPic.ct_voltage.setText("电池状态:" + status + " " + " 电池电压:" + voltage);
                                    }
                                });
                            }
                        }
                        else if (MainActivity.isNavigateOrNot()) {//导航模式
                            navigation_show(dataBuff);

                            if (voltage != 0) {
                                handler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        Navigation.nv_voltage.setText("电池状态:" + status + " " + " 电池电压:" + voltage);
                                    }
                                });
                            }
                        }
                        else if (MainActivity.isStereoFlag()) {//视觉模式
//                            StereoPoint(dataBuff);  //接收双目坐标
//                            receiveStatus(dataBuff);//接收双目状态
                            StereoMessage(dataBuff);

                            if (voltage != 0) {
                                handler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        StereoDisplay.tv_voltage.setText("电池状态:" + status + " " + " 电池电压:" + voltage + "V" + "  小车状态:" + vslamStatus + " ");
                                    }
                                });
                            }
                        }
//                        if (!MainActivity.isReceive_flag() && !MainActivity.isNavigateOrNot())  //接收pbstream文件
//                            ctl_recevPbstream();
                    }
                    try {
                        in.close();
                        ...
                        socket = null;
                        mybitmap = null;
                        if (Navigation.getBitmap() != null)
                            Navigation.setBitmap(null);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    connect = false;
    ...
    private void receiveStatus(byte[] dataBuff) {
        try {

//            in.read(dataBuff, 0, 3);
            if (dataBuff[0] == (byte) 0xaa && dataBuff[1] == (byte) 0x84 && dataBuff[2] == 1) {
                Log.i("tag","————————————————————————开始接收视觉状态");
    ...
    private void StereoMessage(byte[] dataBuff) {
        try {
            if (dataBuff[0] == ((byte) 0xaa) && dataBuff[1] == map_xy.getCmd() && dataBuff[2] == map_xy.getLength()) {//机器人坐标
                Log.i("tag","————————————————————————开始接收机器人坐标");
...
//                    System.out.println("!!!!!!!!!!================"+maxStereoHeight+","+maxStereoWidth+"===================!!!!!!!!!!");
//                    System.out.println("!!!!!!!!!!================"+currentY+","+currentX+"===================!!!!!!!!!!");
                ...
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        if (mybitmap != null)
                            drawStereo(currentX, currentY, currentAngle, maxStereoWidth, maxStereoHeight, mybitmap);

                    }
                });
            }else if (dataBuff[0] == (byte) 0xaa && dataBuff[1] == (byte) 0x84 && dataBuff[2] == 1) {
                Log.i("tag","————————————————————————开始接收视觉状态");
                byte[] temp = new byte[2];
                in.read(temp, 0, 2);
                if (temp[1] == (byte) 0x77) {
                    ...
                    Log.i("tag","————————————————————————！！！接收视觉状态失败！！！");
            }else if (dataBuff[0] == (byte) 0xaa && dataBuff[1] == (byte) 0x83 && dataBuff[2]==(byte)8){
                Log.i("tag","————————————————————————开始接收电量");
                ...
                if (batteryPowerBuff[8] == (byte) 0x77) {
                    ...
                    Log.i("tag","————————————————————————接收电量成功--->状态:"+status+",电压:"+voltage);
                }else
                    Log.i("tag","！！！————————————————————————接收电量失败！！！");
            }
...
//        System.out.println();
        ...
        ArrayList<StereoPoint>list=new ArrayList<>(stereoPoints);
        for (StereoPoint stereoPoint : list)
        ...
        ImageView view = StereoDisplay.get_imageView();
        ...
        try {
            ...
            if (recevBuff[0] == ((byte) 0xaa) && recevBuff[1] == ((byte) 0xfd) && recevBuff[2] == 0) {//pbstream文件
                ...
                if ((int) temp[0] == (int) 0x77) {
                    ...
                    pbstreamFlag = true;
                    Control_getPic.tv_savePbstream.setText("保存成功");
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    pbstreamFlag = false;
                    while (!pbstreamFlag) {
                        sendMessageToCar(TcpMessage.CMD_FUNCTION, TcpMessage.DATA_REQUEST_PBSTREAM);
                        Control_getPic.tv_savePbstream.setText("请求中.." + tempcount++);
                        Thread.sleep(2000);
                    }
                    if (pbstreamFlag)
                        tempcount = 0;
    ...
    private void receive_battery_power() {
        try {
            in.read(batteryPowerHead, 0, 3);
            if (batteryPowerHead[0] == (byte) 0xaa && batteryPowerHead[1] == (byte) 0x83 && batteryPowerHead[2]==(byte)8) {
...
//    }
     ...
     */
    private void ctl_showpic(byte[] dataBuff) {
        try {
            /**
             * 获取相关信息
             */
//            in.read(recevBuff, 0, 3);//此处第三位为长度，自动略过，但还是接收了
//            Log.i("tag","————————————————————————开始接收帧头,"+"0--"+(recevBuff[0]&0xff)+",1--"+(recevBuff[1]&0xff)+",2--"+(recevBuff[2]&0xff));


            if (dataBuff[0] == ((byte) 0xaa) && dataBuff[1] == ((byte) 0xff)) {//地图
                Log.i("tag","————————————————————————开始接收图片");
                ...
                System.out.println("picW="+picW+",picH="+picH+",x="+original_pointX+",y="+original_pointY+",angle="+original_pointAngle);
                /**
                 * 接收图像
                 */
                int offset = 0;
                ...
                System.out.println("数据量应该要有-->"+picH*picW);
                System.out.println("数据量实际量为-->"+offset);
                System.out.println("帧尾为："+temp[0]);
...
//                            Log.i("TcpClient","建图模式接受图像success!");
                    ...
                    Log.i("tag","！！！————————————————————————接收图片失败！！！");
            } else if (dataBuff[0] == ((byte) 0xaa) && dataBuff[1] == map_xy.getCmd() && dataBuff[2] == map_xy.getLength()) {//机器人坐标
                Log.i("tag","————————————————————————开始接收位姿");
                byte[] localxy = new byte[12];
                byte[] back = new byte[1];
                in.read(localxy, 0, 12);
     ...
     */
    private void navigation_show(byte[] dataBuff) {
        try {
            /**
             * 获取相关信息
             */
...
//            if (recevBuff[0] == ((byte) 0xaa) && recevBuff[1] == map_xy.getCmd() && recevBuff[2] == map_xy.getLength()) {//机器人坐标
            ...
            } else if (dataBuff[0] == goalAchieve.getHead() && dataBuff[1] == goalAchieve.getCmd() && dataBuff[2] == goalAchieve.getLength()) {
                Log.i("tag","————————————————————————开始接收成功到达目的地标志位");
     ...
     * @param data
     */
    public void sendMessageToCar(final byte cmd, final byte data) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (out != null) {
                    try {
                        TcpipFunc tcpipFunc = TcpSend.sendMessage(cmd, data);
                        OutputStream outputStream = socket.getOutputStream();
                        outputStream.write(tcpipFunc.send());
                        Log.i("tag", "cmd=" + cmd + ",data=" + data);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                ...
                if (out != null) {
                    try {
                        while (true) {
                            ...
                            speed_local[1] = (byte) 0x02;
                            speed_local[2] = (byte) 2;
                            speed_local[3] = (byte) StereoDisplay.getLx();
                            speed_local[4] = (byte) StereoDisplay.getLy();
                            speed_local[5] = (byte) 0x77;
                            OutputStream outputStream = socket.getOutputStream();
                            outputStream.write(speed_local);
                            Thread.sleep(20);//20ms
                            if (!StereoDisplay.isSendSpeedOrNot())
                                break;
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
            ...
            public void run() {
                if (out != null) {
                    try {
                        ...
                        FloatToByte(lineSpeed, sendSetSpeed.getLineSpeed());
                        FloatToByte(angleSpeed, sendSetSpeed.getAngleSpeed());
...
//                    inputStream.read(data);
                    ...
                    Log.i("tag->>>", "Navigation.getImagePath():" + imagePath);
                    ...
                    byte[] send = new byte[8 + 4 + 4 + 4 + navigationMap.getLength() + pbstreamBuff.length];//
                    send[0] = navigationMap.getHead();    //0:head
                    send[1] = navigationMap.getCmd();     //1:cmd
                    send[2] = 0;  //长度位 默认0
                    for (int temp = 0; temp < 4; temp++) {
                        send[3 + temp] = lenByte[temp];     //2-5:length
                        send[7 + temp] = pbstreamLen[temp];  //6-9: pbstream
                        send[11 + temp] = originX[temp];     //10-13:origin x
                        send[15 + temp] = originY[temp];    //14-17:origin y
                    }
                    for (int temp = 0; temp < fileLength; temp++)
                        send[19 + temp] = data[temp];
                    for (int temp = 0; temp < pbstreamBuff.length; temp++)
                        send[19 + fileLength + temp] = pbstreamBuff[temp];
                    send[19 + fileLength + pbstreamBuff.length] = navigationMap.getBack();//
                    OutputStream outputStream = socket.getOutputStream();
                    outputStream.write(send);
                } catch (IOException e) {
                    e.printStackTrace();
...
//                    inputStream.read(data);
    ...
    */
    ...
    public void draw_current_point(float lx, float ly, float angle, Bitmap bitmap) {
        ...
        Path path = new Path();
        path.moveTo(x + Constant.DOCK_X_GAIN, y);
        path.lineTo(x - Constant.DOCK_X_GAIN, y + Constant.DOCK_Y_GAIN);
        path.lineTo(x - Constant.DOCK_X_GAIN, y - Constant.DOCK_Y_GAIN);
        path.close();
        paint.setColor(Color.RED);
...
//        Log.i("tag", "x=" + x + ",y=" + y);
        ...
        //画站点
        if (Navigation.getNavigation_dockSites() != null) {
            //任务点
            paint.setColor(Color.GREEN);
            for (DockSite dockSite : Navigation.getNavigation_dockSites().values()) {
                ...
                Path path = new Path();
                path.moveTo(dock_x + Constant.DOCK_X_GAIN, dock_y);
                path.lineTo(dock_x - Constant.DOCK_X_GAIN, dock_y + Constant.DOCK_Y_GAIN);
                path.lineTo(dock_x - Constant.DOCK_X_GAIN, dock_y - Constant.DOCK_Y_GAIN);
                path.close();
                canvas.drawPath(path, paint);
                canvas.rotate(dock_angle, dock_x, dock_y);
                canvas.drawText(dockSite.name, dock_x - Constant.DOCK_X_GAIN, dock_y - Constant.DOCK_Y_GAIN, paint);
            }
        }
        //画冲突区域点
        if(Navigation.getConflictAreas()!=null){
            for(ConflictArea conflictArea:Navigation.getConflictAreas()){
                for(Region r:conflictArea.regions){
                    float rx=r.x;
                    float ry=picH -r.y;
                    paint.setColor(0x2361a7ff);
                    canvas.drawCircle(rx,ry, r.r,paint);
                    paint.setColor(Color.BLACK);
                    canvas.drawText(r.name,rx-1,ry-4,paint);
                }
            }
        }
        ...
        Path path1 = new Path();
        path1.moveTo(dx + Constant.DOCK_X_GAIN, dy);
        path1.lineTo(dx - Constant.DOCK_X_GAIN, dy + Constant.DOCK_Y_GAIN);
        path1.lineTo(dx - Constant.DOCK_X_GAIN, dy - Constant.DOCK_Y_GAIN);
        path1.close();
    ...
    private void refreshUI(final boolean isConnected) {
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                MainActivity.getBnConnect().setText(isConnected ? "断开" : "连接AGV");
            }
        });
    }
